...
void ExpandKey(unsigned char key[16], unsigned char* keyExp) {
	// unsigned char* expanded_key = new unsigned char[176];
	//unsigned char keyExp[176];
	for (int i = 0; i < 16; i++) {
		keyExp[i] = key[i];
	}
	...
	while (bytes_count < 176) {
		...
		for (unsigned char a = 0; a < 4; a++) {
			keyExp[bytes_count] = keyExp[bytes_count - 16] ^ temp[a];
			bytes_count++;
		}
	}
	// unsigned char* expanded_key = keyExp;
...
void Encrypt(unsigned char* plaintext, unsigned char* expanded_key, unsigned char* cipher) {
...
void Decrypt(unsigned char* cipher, unsigned char* expanded_key, unsigned char* &plaintext)
{
	unsigned char state[16];
...
unsigned char* string2hex(unsigned char *text, int n, unsigned char* out)
{
	unsigned char mp[100];
    for(int i=0; i<10; i++)
    {
        mp[i + '0'] = i;
    }
	...
	unsigned char reso[n/2];
	for(int i=0; i<n/2; i++)
	{
		unsigned char c1 = text[i*2];
		unsigned char c2 = text[i*2+1];
		int b1 = mp[c1];
		int b2 = mp[c2];
		reso[i] = 16*b1 + b2;
	}
...
void substr(unsigned char *src, int m, int n, unsigned char *dest)
{
    int len = n - m; // cutoff
    int i;
...
int main() {
	...
	unsigned char plaintext[33] = {'6', 'a', 'a', 'd', 'b', 'b', '6', '0', '8', 'e', '5', 'f', '5', '7', 'f', '9', '8', '3', 'd', 'c', '5', '0', 'a', '7', '2', '0', '1', '9', '3', '1', '1', 'd', '\0'};

	unsigned char keytext[33] = {'c','f','7','4','5','7','8','a','b','8','5','3','2','9','0','1','c','f','7','4','5','7','8','a','b','8','5','3','2','9','0','1', '\0'};

	std::cout << "BP: 0 " << std::endl;
	unsigned char key[17];
	string2hex(keytext, 32, key);
	unsigned char expanded_key[177];
	ExpandKey(keytext, expanded_key);
		cout 	<<	"plaintext: "		<< plaintext << endl;
		cout 	<<	"keytext: "		<< keytext << endl;
		cout 	<<	"key: "		<< key << endl;
		cout 	<<	"expanded_key: "		<< expanded_key << endl;
    ...
    for(int part=0; part<(n+31)/32; part++)
    {
        ...
        unsigned char part_string[33];
						std::cout << "BP: 2.0 " << std::endl;
		substr(plaintext, part*32, part*32 + cutoff, part_string);
						std::cout << "BP: 2 " << std::endl;

		for(int i=1; cutoff%32 != 0 && i<32-cutoff%32; i++)
		{
			part_string[33 - i] = '0';
			// part_string += '0';
		}
		...
		unsigned char padded_string[32];
		string2hex(part_string, 32, padded_string);
		unsigned char* cipher;
		Encrypt(padded_string, expanded_key, cipher);
						std::cout << "BP: 4 " << std::endl;
		unsigned char* reverse_cipher;
		Decrypt(cipher, expanded_key, reverse_cipher);
