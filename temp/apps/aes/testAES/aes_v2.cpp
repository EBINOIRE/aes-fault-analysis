...
void string2hex(string text, int n, unsigned char* &res)
{
...
int main() {
	...
	string keytext = "cf74578ab8532901cf74578ab8532901";
	...
	unsigned char* padded_string = (unsigned char*) malloc(128*sizeof(unsigned char));
	unsigned char* cipher = (unsigned char*) malloc(128*sizeof(unsigned char));
	unsigned char*  reverse_cipher = (unsigned char*)0x8022b3f;
	//unsigned char*reverse_cipher = (unsigned char*) malloc(128*((unsigned char)0x8022b3f));
	//reverse_cipher = (unsigned char*)0x8022C3FA;

	while(getline(input, token)) {
		plaintext = token.substr(0, 32);
		...
		for(int part=0; part<(n+31)/32; part++) {
			...
			cipher = Encrypt(padded_string, expanded_key);
			Decrypt(cipher, expanded_key, reverse_cipher);
			string res = hex2string(cipher, 16);
			string dec = hex2string(reverse_cipher, 16);
			total_enc += res;
			total_dec += dec;
			cout<<"mode: " << mode << " Output: " << (mode ? dec : res) <<endl;
			cout<<"Part: "<<part_string<<", AES128: "<<res<<", Decr: "<<dec<<endl;
			output << res << endl;
		}
		cout<<"enc: " << total_enc<<endl<<"dec: "<<total_dec<<endl;
	}
	return 0;
}
